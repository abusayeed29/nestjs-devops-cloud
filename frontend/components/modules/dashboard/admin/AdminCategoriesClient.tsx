...
const navItems = [
  ...
  { label: "Categories",    href: "/admin/categories",      icon: <TagIcon />,    hasChevron: true,
    children: [
...
export function AdminSidebar({ activePage }: { activePage: string }) {
  ...
  const [categoriesOpen, setCategoriesOpen] = useState(activePage === "categories");
    ...
    <aside style={{ width: "210px", minWidth: "210px", background: "#fff", borderRight: "1px solid #e2e8f0", display: "flex", flexDirection: "column", padding: "20px 0" }}>
      ...
      <nav style={{ display: "flex", flexDirection: "column", gap: "1px" }}>
        {navItems.map((item) => {
          ...
          return (
            <div key={item.label}>
              <Link href={isProducts || isCategories ? "#" : item.href}
                onClick={isProducts ? (e) => { e.preventDefault(); setProductsOpen(o => !o); } :
                        isCategories ? (e) => { e.preventDefault(); setCategoriesOpen(o => !o); } : undefined}
                style={{
              ...
              >
                ...
                {item.hasChevron && (
                  <span style={{ color: "#cbd5e1", display: "flex",
                    transform: (isProducts && productsOpen) || (isCategories && categoriesOpen) ? "rotate(180deg)" : "none",
              ...
              {/* Sub-items */}
              {((isProducts && productsOpen) || (isCategories && categoriesOpen)) && item.children && (
...
export function AdminCategoriesClient() {
  ...
  const { getCategories, deleteCategory, categories, meta, isLoading } = useCategory();
  ...
  useEffect(() => {
    ...
    getCategories({ page, limit: 10, search: search || undefined });
      ...
      <main style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e2e8f0", padding: "24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
            <h2 style={{ fontSize: "16px", fontWeight: 700, color: "#0f172a" }}>All Categories</h2>
            <Link href="/admin/categories/add" style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 16px", background: "#3b5bdb", color: "#fff", borderRadius: "8px", fontSize: "13px", fontWeight: 600, textDecoration: "none" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #f1f5f9" }}>
                {["Category", "Slug", "Products", "Status", "Actions"].map((col) => (
            ...
            <tbody>
              {isLoading
                ? Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i}><td colSpan={5} style={{ padding: "14px 12px" }}><div style={{ height: "16px", background: "#f1f5f9", borderRadius: "4px" }} /></td></tr>
                  ))
                : categories.length === 0
                  ? <tr><td colSpan={5} style={{ padding: "40px 12px", textAlign: "center", color: "#94a3b8", fontSize: "13px" }}>No categories found</td></tr>
                  : categories.map((c) => (
                      <tr key={c.id}
                      ...
                      >
                        <td style={{ padding: "12px 12px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            {c.imageUrl
                              ? <img src={c.imageUrl} alt={c.name} style={{ width: "36px", height: "36px", borderRadius: "6px", objectFit: "cover", border: "1px solid #e2e8f0" }} />
                              : <div style={{ width: "36px", height: "36px", borderRadius: "6px", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px" }}>🏷️</div>
                            }
                            <span style={{ fontSize: "13px", fontWeight: 600, color: "#1e293b" }}>{c.name}</span>
                          </div>
                        </td>
                        <td style={{ padding: "12px 12px", fontSize: "12px", color: "#64748b" }}>{c.slug || "—"}</td>
                        <td style={{ padding: "12px 12px", fontSize: "13px", fontWeight: 600, color: "#1e293b" }}>{c.productCount}</td>
                        <td style={{ padding: "12px 12px" }}>
                          <span style={{ padding: "3px 10px", borderRadius: "99px", fontSize: "11px", fontWeight: 700, background: c.isActive ? "#f0fdf4" : "#fef2f2", color: c.isActive ? "#15803d" : "#b91c1c", textTransform: "uppercase" }}>
                            {c.isActive ? "Active" : "Inactive"}
                          </span>
                        </td>
                        <td style={{ padding: "12px 12px" }}>
                          <div style={{ display: "flex", gap: "6px" }}>
                            <Link href={`/admin/categories/${c.id}/edit`} style={{ display: "flex", alignItems: "center", gap: "4px", padding: "5px 10px", background: "#eff6ff", color: "#3b5bdb", borderRadius: "6px", fontSize: "12px", fontWeight: 600, textDecoration: "none" }}>
                              <EditIcon /> Edit
                            </Link>
                            <button
                            ...
                            >
                              <TrashIcon /> {deletingId === c.id ? "…" : "Delete"}
                            </button>
                          </div>
                        </td>
                      </tr>
